...
VERSION_PACKAGE=github.com/gzwillyy/mini/pkg/version
...
.PHONY: all
all: add-copyright lint format build
...
build: tidy # 编译源码，依赖 tidy 目标自动添加/移除依赖包.
	@go build -v -ldflags "$(GO_LDFLAGS)" -o $(OUTPUT_DIR)/mini $(ROOT_DIR)/cmd/mini/main.go
...
ca: ## 生成 CA 文件
	...
	@openssl req -new -key $(OUTPUT_DIR)/cert/ca.key -out $(OUTPUT_DIR)/cert/ca.csr \
		-subj "/C=CN/ST=Guangdong/L=Shenzhen/O=devops/OU=it/CN=127.0.0.1/emailAddress=gzwillyy@gmail.com" # 2. 生成请求文件
	...
	@openssl req -new -key $(OUTPUT_DIR)/cert/server.key -out $(OUTPUT_DIR)/cert/server.csr \
		-subj "/C=CN/ST=Guangdong/L=Shenzhen/O=serverdevops/OU=serverit/CN=127.0.0.1/emailAddress=gzwillyy@gmail.com" # 6. 生成服务端向 CA 申请签名的 CSR
